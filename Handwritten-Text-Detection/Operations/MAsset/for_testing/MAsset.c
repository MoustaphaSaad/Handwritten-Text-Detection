    ...
    mclOutputHandlerFcn print_handler)
{
  ...
  if (!mclmcrInitialize())
    return false;
  if (!GetModuleFileName(GetModuleHandle("MAsset"), path_to_dll, _MAX_PATH))
    return false;
    {
...
{
  return MAssetInitializeWithHandlers(mclDefaultErrorHandler, mclDefaultPrintHandler);
}
